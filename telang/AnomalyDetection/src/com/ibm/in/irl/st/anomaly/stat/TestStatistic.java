...
public class TestStatistic {
	 ...
	 */
	private static final double STAT_THRESHOLD = 3.84; //FL// 95% statistic test --> risk of multiple testing if many clusters

	/**
	 * The field holds the baseline b_c for each cell.
	 * ###FL### PROBLEM: baseline is not so simple in taxi data --> why not simply fit Poisson model as in paper?
	 */
	 ...
	 * ###FL### same distance used for Space and Time neighbors --> maybe to change
	 */
	public void findLocalAnomalies(Vector<Cluster> clusters, GridData grid, Vector<Anomaly> anomalies) {
		...
		int padValue = 2; // neighborhood ~ 2-cell large region around cluster (ST)
						  // same neighborhood distance used for space at time --> could be distinguished
						  // neighbors in time cannot go too far (otherwise, even without anomaly the distribution may change)
						  // same effect but less important for space
		double statValue = 0.0;
		for (Cluster cluster : clusters) {
			findNeighbors(cluster, grid, neighbors, padValue); // what neighbors returned??
			...
			statValue = getLocalStat(clusterValues, neighborValues); // LRT score computing
	...
	public double getLocalStat(Vector<Double> clusterValues, Vector<Double> neighborValues) {
		...
		// ###FL### CELL_BASELINE is useless because is gets cancelled in the calculation
		// ###FL### classic LRT with MLE as in Pang (MLE of iid Poisson parameter = empirical mean)
		//lambdaR = regionSum / (CELL_BASELINE * clusterValues.size());
		...
		lambdaR = regionSum / (clusterValues.size());
		lambdaC = complementSum / (neighborValues.size());
		lambdaG = wholeSum / (neighborValues.size() + clusterValues.size());
