...
public class AnomalyDetector {
	 ...
	 */
	public static void callMe(int i, int r, int c, double gini, String run, int gridRes, String inputDir, String outputDir) throws Exception {
		...
		String inputFile = inputDir + "ct_pd_grid1110_h_"+gridRes+".csv";
		...
		File dir = new File(outputDir + run);
		if (!dir.isDirectory())
		dir.mkdir();
		//Store the grid for evaluation purpose.
		grid.dumpData(outputDir + run + "/GridData.txt");
		...
		Cluster.dumpClusters(clusters, grid.getRows(), grid.getCols(), outputDir + run + "/ClusterCells_"+gini+".txt");
		Cluster.dumpClusterValues(clusters, gridClone, outputDir + run + "/ClusterData_"+gini+".txt");
		Cluster.dumpClusterMap(clusters, grid, outputDir + run + "/ClusterMap_"+gini+".txt");

		//stat.findGlobalAnomalies(clusters, gridDataClone, globalAnomalies);
		Anomaly.dumpAnomalies(anomalies, grid.getRows(), grid.getCols(), outputDir + run + "/AnomalyCells_"+gini+".txt");
		Anomaly.dumpAnomalyValues(anomalies, gridClone, outputDir + run + "/AnomalyData_"+gini+".txt");
		Anomaly.dumpAnomalyMap(anomalies, grid, outputDir + run + "/AnomalyMap_"+gini+".txt");
	...
	public static void main(String[] args) throws Exception {
		...
		int gridRes = 50;
		int intervals = 744; // calculate manually
		
		//Remote
		String inputDir = "/home/vgc/flegros/Data/harish_grid_h/";
		String outputDir = "/home/vgc/flegros/telang/";
		
		
		// Careful with small region --> delete "_s" in filenames for global region
		//Local Small
		//String gridCoordFile = "/home/ferdinand/Documents/NYU/Data/harish_grid_s_h/gridCoordinates_"+ gridRes+ "_s.txt"; 

		String gridCoordFile = inputDir + "gridCoordinates_"+ gridRes+ ".txt";	
		...
		//double[] ginis = { 1E-2, 3E-2, 1E-1, 3E-1 };
		double[] ginis = { 3E-2};
		...
		for (double gini : ginis) {
			...
			callMe(intervals, rows, cols, gini,run,gridRes, inputDir, outputDir);
			System.out.println();
		}
	}
}
