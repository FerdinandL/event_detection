...
{
  this->setParameters(37.8f, 32, 3*8192);
  this->isOrtho = false;
}
...
void Camera3D::updateWorldPositions()
{
  ...
  this->groundRes  = this->getGroundResolution();
...
void Camera3D::glTransform(float dataLevel, const QPointF &translation)
{
  {
...
void Camera3D::saveToFile(const QString &filename)
{
    QFile fo(filename);
    fo.open(QFile::WriteOnly | QIODevice::Text);
    QTextStream output(&fo);

    QVector3DD eye = this->pEye;
    output << eye.x() << " " << eye.y() << " " << eye.z() << "\n";

    QVector3DD lkt = this->pLookAt;
    output << lkt.x() << " " << lkt.y() << " " << lkt.z() << "\n";

    QVector3DD up = this->pUp;
    output << up.x() << " " << up.y()   << " " << up.z()  << "\n";
